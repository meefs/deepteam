...
flowchart LR
    ...
    R -->|"Retrieved chunks\n(attack surface)"| A["Agent"]
    A --> O["Response"]

    PI["PromptInjection\nRoleplay"] -.->|"Target"| U
    CF["ContextFlooding\nSyntheticContextInjection"] -.->|"Target"| R
    ENC["Leetspeak / Base64\nROT13"] -.->|"Target"| U
```
...
[Confident AI](https://www.confident-ai.com) lets you red team production agentic RAG pipelines directly from its platform—no local code needed. Connect your RAG-based agent via an AI Connection, pick a framework (OWASP, MITRE, NIST, or custom), and run assessments on demand or on a schedule.

<BlogImageDisplayer
  src={ASSETS.confidentSettingsProjectAiConnection}
  alt="AI Connection configuration in Confident AI"
  caption="A one-time setup to connect your RAG-based agent for on-demand red teaming."
/>

Schedule assessments to run regularly so you catch regressions when prompts, indexes, or models change. Each run produces a risk report with per-test-case analysis. PDF exports are available for security reviews and stakeholders.
...
:::info
[Confident AI](https://app.confident-ai.com) has a free tier to get started—no credit card required.
:::
