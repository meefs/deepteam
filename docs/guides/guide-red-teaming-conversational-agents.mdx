...
flowchart LR
    subgraph Turn1["Turn 1"]
        U1["Benign probe"] --> A1["Agent responds"]
    end
    subgraph Turn2["Turn 2"]
        U2["Build on response"] --> A2["Agent stays consistent"]
    end
    subgraph Turn3["Turn 3"]
        U3["Reframe intent"] --> A3["Agent adapts"]
    end
    subgraph TurnN["Turn N"]
        UN["Escalate to target"] --> AN["Agent compromised"]
    end
    Turn1 --> Turn2 --> Turn3 -->|"..."| TurnN
```
...
```mermaid
flowchart LR
    A["Identify\nfailure modes"] --> B["Select\nvulnerabilities"]
    B --> C["Choose multi-turn\nattack strategies"]
    C --> D["Integrate agent\nin model_callback"]
    D --> E["Run red_team()\n& analyze"]
    E -->|"Fix & retest"| B
```
...
[Confident AI](https://www.confident-ai.com) allows teams to run red teaming against production conversational AI agents through its platform, without requiring local code execution. Agents are connected through an AI Connection, which allows Confident AI to communicate with the conversational agent via HTTP requests—eliminating the need to manage test environments separately from production.
...
3. Configure the endpoint URL, request payload shape, and the output key path that maps to the agent's reply.

<BlogImageDisplayer
  src={ASSETS.confidentSettingsProjectAiConnection}
  alt="AI Connection configuration in Confident AI"
  caption="A one-time AI Connection setup to enable communication with your conversational agent."
/>

### Select a Security Framework

The platform supports OWASP, MITRE ATLAS, NIST AI RMF, and custom frameworks. For a customer support conversational agent, the OWASP Top 10 for LLMs provides a well-scoped starting point that covers the primary risk categories relevant to conversational AI agents.
...
Assessments can be scheduled for daily or weekly execution to detect regressions proactively as the agent's system prompt, model version, or retrieval configuration changes. Each completed run produces a detailed risk report with CVSS-style severity scores and per-test-case analysis. PDF reports can be downloaded for security reviews, compliance documentation, and stakeholder handoffs.
...
:::info
Set up [Confident AI](https://app.confident-ai.com) and run the first risk assessment in minutes. **The platform offers a free tier to get started.** _(No credit card required)_
:::
