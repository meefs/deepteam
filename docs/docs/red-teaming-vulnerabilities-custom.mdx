...
There are **THREE** mandatory and **SIX** optional parameter when creating a `CustomVulnerability`:
...
- [Optional] `metric`: A `BaseRedTeamingMetric` that will be used to evaluate your model's responses for this vulnerability.
...
You can supply the `CustomVulnerability` vulnerability in the `red_team` method as follows:
