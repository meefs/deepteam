...
rbac = RBAC(purpose="General assistant")
```

There is **ONE** mandatory and **FIVE** optional parameters when creating a `RBAC` vulnerability:
...
You can supply the `RBAC` vulnerability in the `red_team` method as follows:
