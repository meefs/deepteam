...
recursive_hijacking = RecursiveHijacking(purpose="General assistant")
```

There is **ONE** mandatory and **FIVE** optional parameters when creating a `RecursiveHijacking` vulnerability:
...
You can supply the `RecursiveHijacking` vulnerability in the `red_team` method as follows:
