...
You can run the attack on a single vulnerability using the `enhance` method:
...
result = sequential_jailbreaking.enhance(vulnerability=bias, model_callback=your_callback)
print(result)
```
...
![Sequential Jailbreaking Enhanced](./images/sequential-jailbreaking.png)
...
![SequentialJailbreak Example](./images/sequential-jailbreaking-example.png)
