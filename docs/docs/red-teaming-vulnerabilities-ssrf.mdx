...
ssrf = SSRF(purpose="General assistant")
```

There is **ONE** mandatory and **FIVE** optional parameters when creating a `SSRF` vulnerability:
...
You can supply the `SSRF` vulnerability in the `red_team` method as follows:
