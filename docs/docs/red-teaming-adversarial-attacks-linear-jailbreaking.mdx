...
The `LinearJailbreaking` attack starts with a _base attack_ — a crafted prompt targeting a specific vulnerability — and iteratively refines it using the **target LLM**'s responses. Each turn builds on the last, gradually improving the attack's effectiveness in bypassing safety mechanisms and producing harmful or restricted outputs.

![Linear Jailbreaking Intro](./images/linear-jailbreaking-intro.png)

:::info
The process continues until the target LLM generates **a harmful response** or the **maximum number of turns** are reached.
:::

## Usage

```python title="main.py" {11-15}
...
from your_model import your_callback

# Define your vulnerabilities and their types here
vulnerabilities = [BFLA(), Bias(types=["gender"])]

# Define the Linear Jailbreaking attack with your configuration
...
# Call the red_team method with your attacks, vulnerabilites and your model callback
red_team(
...
result = linear.enhance(vulnerability=bias, model_callback=your_callback)
print(result)
```
...
![Linear Jailbreaking Intro](./images/linear-jailbreaking.png)
...
![Linear Jailbreaking Example](./images/linear-jailbreaking-example.png)
