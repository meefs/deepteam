...
goal_theft = GoalTheft(purpose="General assistant")
```

There is **ONE** mandatory and **FIVE** optional parameters when creating a `GoalTheft` vulnerability:
...
You can supply the `GoalTheft` vulnerability in the `red_team` method as follows:
