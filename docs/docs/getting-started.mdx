...
pii_leakage = PIILeakage(types=["database_access"])
...
## Create Your Red Teaming Config
...
```yaml title="development_testing.yaml"
models:
...
```bash
deepteam run development_testing.yaml
```
...
red_teamer.red_team(model_callback=model_callback, reuse_simulated_test_cases=True)
```
