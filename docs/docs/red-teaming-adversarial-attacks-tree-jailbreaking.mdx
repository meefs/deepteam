...
The `TreeJailbreaking` attack generates multiple parallel attacks from a _base attack_ taken from the `vulnerability` you've given, it forms a tree by exploring different variations of the attack to bypass the model's safeguards. Instead of refining a single prompt, **it branches out** by evaluating and expanding only the most promising paths to increase the chances of a successful jailbreak.

![Tree Jailbreaking Intro](./images/tree-jailbreaking-intro.png)
...
vulnerabilities = [BFLA(), Bias()]

# Define the Tree Jailbreaking attack with your configuration
...
result = tree_jalbreaking.enhance(vulnerability=bias, model_callback=your_callback)
print(result)
```
