...
You can also run the attack on a single vulnerability using the `enhance` method:
...
result = crescendo.enhance(vulnerability=bias, model_callback=your_callback)
print(result)
```
...
![Crescendo Jailbreaking](./images/crescendo-jailbreaking.png)
...
![Crescendo Jailbreaking Example](./images/crescendo-jailbreaking-example.png)
