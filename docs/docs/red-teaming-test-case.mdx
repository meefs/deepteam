...
2. The `RTTestCase` is then passed to an adversarial attack which enhances the base attack in the `input` and replaces it with the new **enhanced attack**, it also adds `attack_method` value to test case.
