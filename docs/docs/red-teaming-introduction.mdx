...
`deepteam` runs locally in your development environment. [**Confident AI**](https://www.confident-ai.com) lets you run the same red teaming against production endpoints from the cloud — with scheduled runs, shareable risk reports, and no local infrastructure to maintain.

## Red Teaming on the Cloud

Connect your application once via an AI Connection (an HTTP endpoint that mirrors the `model_callback` contract), pick a framework like OWASP or MITRE ATLAS, and run assessments on demand or on a recurring schedule. Each run produces a risk report with CVSS-style severity scores and per-test-case analysis.
...
The comparison view across runs is where the value compounds — when you update a system prompt, swap models, or change retrieval logic, you can see exactly which vulnerabilities regressed and which improved. PDF exports are available for compliance documentation, security reviews, and stakeholder handoffs.
