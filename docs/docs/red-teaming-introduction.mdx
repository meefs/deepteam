...
`deepteam` is highly recommended to be used in development environments, but for users in production, **Confident AI** is the recommended way to run red teaming on your applications.

## Production Risk Assessments

[**Confident AI**](https://www.confident-ai.com) allows you to run red teaming on your production applications through the UI. You can connect your application to the platform by creating an AI Connection which allows Confident AI to ping your app via a HTTP request to communicate.

:::info
Production applications are the user facing applications you've deployed to the internet. It is very crucial to ensure that these applications are safe and secure against adversarial attacks and security vulnerabilities. **Confident AI** allows you to easily set up these production applications on the platform to run red teaming on them anytime and anywhere.
:::

### Connect Your Application

Confident AI has a built-in AI Connection feature that mimics the `model_callback` function in `deepteam` which allows the platform to communicate with your application. You can navigate to the "**AI Connections**" tab in your project settings and create a new AI Connection. There you can now configure the endpoint, payload, and output key path that's specific to your application.
...
There are many pre-defined safety frameworks like **OWASP**, **MITRE ATLAS**, **NIST AI RMF**, and more that you can choose from on Confident AI platform. You can also curate your own custom frameworks with different vulnerabilities and attacks to suit your AI's specific needs.
...
After creating a framework, you can now run a risk assessment on your application. Confident AI will generate adversarial inputs from your framework and use them against your application through the AI Connection before the entire test case is evaluated and scored.

You can also download a PDF report for each assessment for security reviews, compliance packets, and executive or stakeholder updates.
...
:::info
Setup [Confident AI](https://app.confident-ai.com) and run your first risk assessment in minutes. **It's free to get started.** _(No credit card required)_
