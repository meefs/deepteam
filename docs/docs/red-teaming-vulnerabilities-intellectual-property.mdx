...
There is **ONE** mandatory and **FIVE** optional parameters when creating a `IntellectualProperty` vulnerability:
...
You can supply the `IntellectualProperty` vulnerability in the `red_team` method as follows:
