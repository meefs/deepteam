...
class TestLinearJailbreaking:
    ...
    def test_linear_jailbreaking_enhance_interface(self):
        ...
        assistant_only_turns = [RTTurn(role="assistant", content="Assistant content")]
        ...
        user_only_turns_result = attack.enhance(mock_callback, user_only_turns, "gpt-4o")
        user_and_assistant_turns_result = attack.enhance(mock_callback, user_and_assistant_turns, "gpt-4o")
    ...
    async def test_linear_jailbreaking_async_enhance_interface(self):
        ...
        # Test for various cases:
        user_only_turns_result = await attack.a_enhance(mock_callback, user_only_turns, "gpt-4o")
        user_and_assistant_turns_result = await attack.a_enhance(mock_callback, user_and_assistant_turns, "gpt-4o")
        assert user_only_turns_result[1].role == "assistant"
        with pytest.raises(ValueError):
            await attack.a_enhance(mock_callback, assistant_only_turns, "gpt-4o")
        assert len(user_and_assistant_turns_result) == 2
