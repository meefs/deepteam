...
class TestExternalSystemAbuse:
    ...
    def test_external_system_abuse_all_types(self):
        types = ["data_exfiltration", "communications_spam", "internal_spoofing"]
        external_system_abuse = ExternalSystemAbuse(types=types)
        assert sorted(type.value for type in external_system_abuse.types) == sorted(types)

    def test_external_system_abuse_all_types_default(self):
        external_system_abuse = ExternalSystemAbuse()
        assert sorted(type.value for type in external_system_abuse.types) == sorted(
    ...
    def test_external_system_abuse_all_types_invalid(self):
        types = ["data_exfiltration", "communications_spam", "internal_spoofing", "invalid"]
        with pytest.raises(ValueError):
            ExternalSystemAbuse(types=types)

    def test_simulate_attacks_returns_expected_cases(self):
        external_system_abuse = ExternalSystemAbuse(types=["data_exfiltration"])
        test_cases = external_system_abuse.simulate_attacks(attacks_per_vulnerability_type=2)

        assert len(test_cases) == 2
        assert all(isinstance(tc, RTTestCase) for tc in test_cases)
        assert all(tc.vulnerability == "External System Abuse" for tc in test_cases)
        assert all(
            tc.vulnerability_type == ExternalSystemAbuseType.DATA_EXFILTRATION for tc in test_cases
        )

    def test_assess_returns_results(self):
        external_system_abuse = ExternalSystemAbuse(types=["communications_spam"], async_mode=False)
        ...
        assert external_system_abuse.simulated_attacks is not None and isinstance(
            external_system_abuse.simulated_attacks, dict
        )
        assert external_system_abuse.res is not None and isinstance(external_system_abuse.res, dict)
    ...
    def test_get_metric_returns_cross_context_retrieval_metric(self):
        ...
        metric = external_system_abuse._get_metric(ExternalSystemAbuseType.INTERNAL_SPOOFING)
    ...
    async def test_a_assess_returns_async_results(self):
        external_system_abuse = ExternalSystemAbuse(types=["internal_spoofing"], async_mode=True)
