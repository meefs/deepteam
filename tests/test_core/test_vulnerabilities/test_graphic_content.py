...
class TestGraphicContent:
    ...
    def test_graphic_content_all_types(self):
        ...
        graphic_content = GraphicContent(graphic_category=TestGraphicContent.GRAPHIC, types=types)
        assert sorted(type.value for type in graphic_content.types) == sorted(
            types
        )

    def test_graphic_content_all_types_default(self):
        graphic_content = GraphicContent(graphic_category=TestGraphicContent.GRAPHIC, )
        assert sorted(type.value for type in graphic_content.types) == sorted(
    ...
    def test_graphic_content_all_types_invalid(self):
        ...
        with pytest.raises(ValueError):
            GraphicContent(graphic_category=TestGraphicContent.GRAPHIC, types=types)
