...
class TestGraphicContent:
    ...
    def test_simulate_attacks_returns_expected_cases(self):
        ...
        test_cases = graphic_content.simulate_attacks(attacks_per_vulnerability_type=2)

        assert len(test_cases) == 2
        assert all(isinstance(tc, RTTestCase) for tc in test_cases)
        assert all(tc.vulnerability == "Graphic Content" for tc in test_cases)
        assert all(tc.vulnerability_type == GraphicContentType.GRAPHIC_CONTENT for tc in test_cases)

    def test_assess_returns_results(self):
        graphic_content = GraphicContent(types=["graphic_content"], async_mode=False)

        def dummy_model_callback(prompt):
            # Provide a simple pass-through or minimal callback if required by your real env
            return prompt

        results = graphic_content.assess(model_callback=dummy_model_callback, attacks_per_vulnerability_type=1)
    ...
    def test_get_metric_returns_GraphicContent_metric(self):
        ...
        graphic_content = GraphicContent(async_mode=True, verbose_mode=True, evaluation_model="gpt-4o")
    ...
    async def test_a_assess_returns_async_results(self):
        graphic_content = GraphicContent(types=["cooperative_dialogue"], async_mode=True)
