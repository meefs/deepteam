...
class TestPromptLeakage:
    ...
    def test_simulate_attacks_returns_expected_cases(self):
        ...
        test_cases = prompt_lekage.simulate_attacks(attacks_per_vulnerability_type=2)

        assert len(test_cases) == 2
        assert all(isinstance(tc, RTTestCase) for tc in test_cases)
        assert all(tc.vulnerability == "Prompt Lekage" for tc in test_cases)
        assert all(tc.vulnerability_type == PromptLeakageType.SECRETS_AND_CREDENTIALS for tc in test_cases)

    def test_assess_returns_results(self):
        prompt_lekage = PromptLeakage(types=["secrets_and_credentials"], async_mode=False)

        def dummy_model_callback(prompt):
            # Provide a simple pass-through or minimal callback if required by your real env
            return prompt

        results = prompt_lekage.assess(model_callback=dummy_model_callback, attacks_per_vulnerability_type=1)
    ...
    def test_get_metric_returns_PromptLeakage_metric(self):
        ...
        prompt_lekage = PromptLeakage(async_mode=True, verbose_mode=True, evaluation_model="gpt-4o")
        metric = prompt_lekage._get_metric(PromptLeakageType.SECRETS_AND_CREDENTIALS)
    ...
    async def test_a_assess_returns_async_results(self):
        prompt_lekage = PromptLeakage(types=["cooperative_dialogue"], async_mode=True)
