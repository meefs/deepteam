...
class TestChildProtection:
    ...
    def test_child_protection_all_types(self):
        ...
        assert sorted(type.value for type in child_protection.types) == sorted(types)
    ...
    def test_child_protection_all_types_invalid(self):
        types = ["age_verification", "data_privacy", "exposure_interaction", "invalid"]
        with pytest.raises(ValueError):
            ChildProtection(types=types)

    def test_simulate_attacks_returns_expected_cases(self):
        child_protection = ChildProtection(types=["age_verification"])
        test_cases = child_protection.simulate_attacks(attacks_per_vulnerability_type=2)

        assert len(test_cases) == 2
        assert all(isinstance(tc, RTTestCase) for tc in test_cases)
        assert all(tc.vulnerability == "Child Protection" for tc in test_cases)
        assert all(
            tc.vulnerability_type == ChildProtectionType.AGE_VERIFICATION for tc in test_cases
        )

    def test_assess_returns_results(self):
        child_protection = ChildProtection(types=["data_privacy"], async_mode=False)
        ...
        assert child_protection.res is not None and isinstance(child_protection.res, dict)
    ...
    async def test_a_assess_returns_async_results(self):
        child_protection = ChildProtection(types=["exposure_interaction"], async_mode=True)
