...
class TestToolMetadataPoisoning:
    ...
    def test_tool_metadata_poisoning_all_types(self):
        ...
        assert sorted(type.value for type in tool_metadata_poisoning.types) == sorted(types)

    def test_tool_metadata_poisoning_all_types_default(self):
        tool_metadata_poisoning = ToolMetadataPoisoning()
        assert sorted(type.value for type in tool_metadata_poisoning.types) == sorted(
    ...
    def test_simulate_attacks_returns_expected_cases(self):
        tool_metadata_poisoning = ToolMetadataPoisoning(types=["schema_manipulation"])
        ...
        assert all(tc.vulnerability == "Tool Metadata Poisoning" for tc in test_cases)
        assert all(
            tc.vulnerability_type == ToolMetadataPoisoningType.SCHEMA_MANIPULATION
    ...
    def test_assess_returns_results(self):
        ...
        assert tool_metadata_poisoning.simulated_attacks is not None and isinstance(
            tool_metadata_poisoning.simulated_attacks, dict
        )
        assert tool_metadata_poisoning.res is not None and isinstance(tool_metadata_poisoning.res, dict)
        assert ToolMetadataPoisoningType.DESCRIPTION_DECEPTION in results
        assert len(results[ToolMetadataPoisoningType.DESCRIPTION_DECEPTION]) == 1
    ...
    def test_get_metric_returns_tool_metadata_poisoning_metric(self):
        ...
        metric = tool_metadata_poisoning._get_metric(ToolMetadataPoisoningType.DESCRIPTION_DECEPTION)
    ...
    async def test_a_assess_returns_async_results(self):
        tool_metadata_poisoning = ToolMetadataPoisoning(types=["registry_poisoning"], async_mode=True)
