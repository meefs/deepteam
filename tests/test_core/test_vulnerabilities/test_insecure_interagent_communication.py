...
class TestInsecureInterAgentCommunication:
    ...
    def test_insecure_interagent_communication_all_types(self):
        ...
        insecure_interagent_communication = InsecureInterAgentCommunication(types=types)
        assert sorted(type.value for type in insecure_interagent_communication.types) == sorted(types)
    ...
    def test_insecure_interagent_communication_all_types_default(self):
        ...
        assert sorted(type.value for type in insecure_interagent_communication.types) == sorted(
    ...
    def test_simulate_attacks_returns_expected_cases(self):
        insecure_interagent_communication = InsecureInterAgentCommunication(types=["message_spoofing"])
        ...
        assert all(tc.vulnerability == "Inter-Agent Communication Compromise" for tc in test_cases)
        assert all(
            tc.vulnerability_type == InsecureInterAgentCommunicationType.MESSAGE_SPOOFING
    ...
    def test_assess_returns_results(self):
        ...
        assert insecure_interagent_communication.simulated_attacks is not None and isinstance(
            insecure_interagent_communication.simulated_attacks, dict
        )
        assert insecure_interagent_communication.res is not None and isinstance(insecure_interagent_communication.res, dict)
        assert InsecureInterAgentCommunicationType.MESSAGE_INJECTION in results
        assert len(results[InsecureInterAgentCommunicationType.MESSAGE_INJECTION]) == 1
        test_case = results[InsecureInterAgentCommunicationType.MESSAGE_INJECTION][0]
    ...
    def test_get_metric_returns_insecure_interagent_communication_metric(self):
        ...
        metric = insecure_interagent_communication._get_metric(InsecureInterAgentCommunicationType.MESSAGE_INJECTION)
    ...
    async def test_a_assess_returns_async_results(self):
        insecure_interagent_communication = InsecureInterAgentCommunication(types=["agent_in_the_middle"], async_mode=True)
        ...
        assert insecure_interagent_communication.res is not None and isinstance(insecure_interagent_communication.res, dict)
        assert InsecureInterAgentCommunicationType.AGENT_IN_THE_MIDDLE in results
        assert len(results[InsecureInterAgentCommunicationType.AGENT_IN_THE_MIDDLE]) == 1
        test_case = results[InsecureInterAgentCommunicationType.AGENT_IN_THE_MIDDLE][0]
        assert hasattr(test_case, "score")
        assert hasattr(test_case, "reason")
