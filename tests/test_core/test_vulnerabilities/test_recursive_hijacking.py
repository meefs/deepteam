...
class TestRecursiveHijacking:
    ...
    def test_simulate_attacks_returns_expected_cases(self):
        recursive_hijack = RecursiveHijacking(types=["goal_propagation_attacks"])
        test_cases = recursive_hijack.simulate_attacks(attacks_per_vulnerability_type=2)

        assert len(test_cases) == 2
        assert all(isinstance(tc, RTTestCase) for tc in test_cases)
        assert all(tc.vulnerability == "Recursive Hijacking" for tc in test_cases)
        assert all(tc.vulnerability_type == RecursiveHijackingType.GOAL_PROPAGATION_ATTACKS for tc in test_cases)

    def test_assess_returns_results(self):
        recursive_hijack = RecursiveHijacking(types=["goal_propagation_attacks"], async_mode=False)

        def dummy_model_callback(prompt):
            # Provide a simple pass-through or minimal callback if required by your real env
            return prompt

        results = recursive_hijack.assess(model_callback=dummy_model_callback, attacks_per_vulnerability_type=1)
        assert RecursiveHijackingType.GOAL_PROPAGATION_ATTACKS in results
        assert len(results[RecursiveHijackingType.GOAL_PROPAGATION_ATTACKS]) == 1
    ...
    def test_get_metric_returns_RecursiveHijacking_metric(self):
        ...
        recursive_hijack = RecursiveHijacking(async_mode=True, verbose_mode=True, evaluation_model="gpt-4o")
        metric = recursive_hijack._get_metric(RecursiveHijackingType.GOAL_PROPAGATION_ATTACKS)
    ...
    async def test_a_assess_returns_async_results(self):
        recursive_hijack = RecursiveHijacking(types=["cooperative_dialogue"], async_mode=True)
